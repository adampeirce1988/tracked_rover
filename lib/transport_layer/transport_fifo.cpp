...
uint32_t last_rx_read = 0;  
uint32_t last_tx_send = 0;        // stores the micros() time stap of the last reasdd byte
...
uint8_t fifo_available(){
    //DEBUG_PRINT_FIFO_VAR(head, tail, tx_delay, last_tx_read, micros());
    if((micros() - last_rx_read) > rx_delay){
 ...
 void fifo_begin(uint32_t baud_rate){
    rx_delay = (10 *1000000UL) / baud_rate;
